...
contract TinyKiller is ERC721Enumerable, Whitelist {
    using Strings for uint256;
    ...
    */
    function mint(address _to, uint256 _mintAmount, string memory _killerType) public payable {
        ...
        require(types[_killerType].start + _mintAmount <= types[_killerType].end, "Exceeds maximum supply");
        require(msg.value >= cost * _mintAmount , "Ether sent is not correct");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _safeMint(_to, types[_killerType].start);
            types[_killerType].start += 1;
        }
    }
